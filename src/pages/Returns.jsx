...
const getStatusClass = (status) =>
  `returns-status returns-status--${status.toLowerCase()}`;

const SearchIcon = () => (
  <span className="returns-search-icon" aria-hidden="true">
    ⌕
  </span>
);
const AlertIcon = () => (
...
function Returns() {
  ...
  const pendingRecords = detailedLoans.filter(
    (record) => record.status === "Active" || record.status === "Overdue",
  );
  const overdueRecords = pendingRecords.filter(
    (record) => record.status === "Overdue",
  );
  const unpaidFineRecords = detailedLoans.filter(
    (record) => record.fine > 0 && record.fineStatus === "Unpaid",
  );
  const pendingFines = unpaidFineRecords.reduce(
    (total, record) => total + record.fine,
    0,
  );
  ...
  return (
    <>
      <style>{`
        ...
        .returns-controls { display: flex; justify-content: space-between; align-items: end; gap: 16px; padding: 20px 24px 0; flex-wrap: wrap; } .returns-search { display: flex; align-items: center; gap: 10px; flex: 1 1 560px; min-width: 260px; max-width: none; padding: 10px 14px; border: 1px solid #dfe7ef; border-radius: 10px; background: #f8fafc; } .returns-search:focus-within { border-color: #dfe7ef; box-shadow: none; } .returns-search-icon { color: #8193a6; font-size: 20px; line-height: 1; } .returns-search input { width: 100%; border: 0; outline: none; background: transparent; color: #102334; font: inherit; font-size: 14px; } .returns-filter { display: flex; flex-direction: column; gap: 8px; min-width: 180px; color: #52677d; font-size: 12px; font-weight: 600; } .returns-filter select { padding: 10px 12px; border: 1px solid #dfe7ef; border-radius: 8px; background: #f8fafc; color: #102334; font: inherit; }
        .returns-search:focus-within { border-color: #60a5fa; box-shadow: 0 0 0 3px rgba(96,165,250,.16); }
      ...
      <div className="returns-page">
        <div className="returns-panel">
          <div className="returns-heading">
            ...
            <p>Track active loans, overdue books, and outstanding fines.</p>
          </div>
          <section className="returns-summary" aria-label="Return summary">
            <div className="returns-summary-card">
              <span className="returns-summary-icon returns-summary-icon--books">
                <BooksSummaryIcon />
              </span>
              <div>
                <p className="returns-summary-label">Books Checked Out</p>
                <p className="returns-summary-value">{pendingRecords.length}</p>
                <p className="returns-summary-helper">Currently on loan</p>
              </div>
            </div>
            <div className="returns-summary-card returns-summary-card--overdue">
              <span className="returns-summary-icon returns-summary-icon--overdue">
                <OverdueSummaryIcon />
              </span>
              <div>
                <p className="returns-summary-label">Overdue Books</p>
                <p className="returns-summary-value">{overdueRecords.length}</p>
                <p className="returns-summary-helper">
                  Need immediate attention
                </p>
              </div>
            </div>
            <div className="returns-summary-card returns-summary-card--fine">
              <span className="returns-summary-icon returns-summary-icon--fine">
                <FineSummaryIcon />
              </span>
              <div>
                <p className="returns-summary-label">Total Pending Fines</p>
                <p className="returns-summary-value">
                  ${pendingFines.toFixed(2)}
                </p>
                <p className="returns-summary-helper">
                  {unpaidFineRecords.length} unpaid{" "}
                  {unpaidFineRecords.length === 1 ? "fine" : "fines"}
                </p>
              </div>
            </div>
          </section>
          {overdueRecords.length > 0 && (
            <div className="returns-alert">
              <AlertIcon />
              {overdueRecords.length} overdue books — please contact the
              respective students.
            </div>
          )}
          <div className="returns-controls">
            <label className="returns-search">
              ...
              <input
                ...
                aria-label="Search pending returns"
              />
            </label>
              ...
              <tbody>
                {visibleRecords.length === 0 ? (
                  ...
                  visibleRecords.map((record) => {
                    ...
                    const daysLabel =
                      record.status === "Overdue"
                        ? `${record.overdueDays} ${record.overdueDays === 1 ? "day" : "days"} late`
                        : record.days === 0
                          ? "Due today"
                          : `${record.days} ${record.days === 1 ? "day" : "days"} remaining`;
                    return (
                      <tr key={record.loanId}>
                        <td>{record.loanId}</td>
                        <td>
                          <div className="returns-student">
                            <span className="returns-avatar" aria-hidden="true">
                              {initials}
                            </span>
                            <div className="returns-cell-main">
                        ...
                        <td className={daysClass}>{daysLabel}</td>
                        <td
                          className={record.fine ? "returns-fine--overdue" : ""}
                        >
                        ...
                        <td>
                          <button
                            ...
                            aria-label={`Return ${record.bookTitle} from ${record.studentName}`}
                          >
                            Return
                          </button>
                        </td>
          ...
          <footer className="returns-footer">
            <span>
              ...
              -{Math.min(displayPage * PAGE_SIZE, filteredRecords.length)} of{" "}
              {filteredRecords.length} returns
            </span>
            <div className="returns-pagination" aria-label="Returns pagination">
              <button
                type="button"
                className="returns-page-button"
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1).map(
                (page) => (
                  <button
                    type="button"
                    key={page}
                    className={`returns-page-button ${page === displayPage ? "returns-page-button--active" : ""}`}
      ...
      {selectedRecord && (
        <div
          className="returns-modal-backdrop"
        ...
        >
          ...
          >
            <div className="returns-modal-header">
              <h3 id="return-dialog-title">Confirm Book Return</h3>
              <button
                type="button"
                className="returns-modal-close"
                onClick={() => setSelectedRecord(null)}
            ...
            <div className="returns-modal-content">
              <p>
                Return <strong>{selectedRecord.bookTitle}</strong> from{" "}
                <strong>{selectedRecord.studentName}</strong>?
              </p>
              <p>
                Return date: <strong>{formatDate(getDateOnly())}</strong>
              </p>
              {selectedRecord.fine > 0 && (
                <p className="returns-modal-fine">
                  This book is {selectedRecord.overdueDays} days overdue.
                  Applicable fine: ${selectedRecord.fine.toFixed(2)} (Unpaid).
                </p>
              )}
            </div>
