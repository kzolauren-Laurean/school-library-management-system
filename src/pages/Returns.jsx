...
function Returns() {
  ...
  const pendingFines = unpaidFineRecords.reduce((total, record) => total + record.fine, 0);
  ...
  const visibleRecords = filteredRecords.slice((displayPage - 1) * PAGE_SIZE, displayPage * PAGE_SIZE);
  ...
  return (
    <>
      ...
      <div className="returns-page"><div className="returns-panel">
        ...
        <div className="returns-controls"><label className="returns-search"><SearchIcon /><input value={searchTerm} onChange={(event) => { setSearchTerm(event.target.value); setCurrentPage(1); }} placeholder="Search by student or book..." aria-label="Search pending returns" /></label><label className="returns-filter">Status<select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setCurrentPage(1); }} aria-label="Filter returns by status"><option>All Status</option><option>Active</option><option>Overdue</option></select></label></div>
        ...
        <footer className="returns-footer"><span>Showing {filteredRecords.length === 0 ? 0 : (displayPage - 1) * PAGE_SIZE + 1}-{Math.min(displayPage * PAGE_SIZE, filteredRecords.length)} of {filteredRecords.length} returns</span><div className="returns-pagination" aria-label="Returns pagination"><button type="button" className="returns-page-button" disabled={displayPage === 1} onClick={() => setCurrentPage((page) => page - 1)}>Prev</button>{Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => <button type="button" key={page} className={`returns-page-button ${page === displayPage ? "returns-page-button--active" : ""}`} onClick={() => setCurrentPage(page)}>{page}</button>)}<button type="button" className="returns-page-button" disabled={displayPage === totalPages} onClick={() => setCurrentPage((page) => page + 1)}>Next</button></div></footer>
      </div></div>
