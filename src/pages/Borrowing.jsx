...
function Borrowing() {
  ...
  return (
    <>
      ...
      .borrowing-heading h2 { margin: 0; color: #20364a; font-size: 20px; font-weight: 700; line-height: 1.3; }
      .borrowing-heading p { margin: 6px 0 0; color: #8a9bad; font-size: 13px; }
      .borrowing-add-button { display: inline-flex; align-items: center; gap: 8px; min-height: 40px; padding: 0 18px; border: 0; border-radius: 8px; background: #334e68; color: #fff; font: inherit; font-size: 14px; font-weight: 600; cursor: pointer; transition: background .2s, transform .15s; }
      .borrowing-add-button:hover { background: #2b4257; transform: translateY(-1px); }
      .borrowing-add-button:focus-visible, .borrowing-return-button:focus-visible, .borrowing-page-button:focus-visible, .borrowing-close:focus-visible { outline: 2px solid #334e68; outline-offset: 2px; }
      .borrowing-summary { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; padding: 20px 24px 0; }
      .borrowing-summary-card { display: flex; align-items: center; gap: 16px; min-width: 0; padding: 17px 19px; border: 1px solid #e5edf5; border-radius: 12px; background: #f8fbff; }
      .borrowing-summary-card--active { border-color: #d7eadf; background: #f7fcf8; }
      .borrowing-summary-card--overdue { border-color: #f3d9d9; background: #fffafa; }
      .borrowing-summary-card--due { border-color: #f2e3c7; background: #fffdf8; }
      .borrowing-summary-icon { display: grid; place-items: center; flex: 0 0 56px; width: 56px; height: 56px; border-radius: 50%; }
      .borrowing-summary-icon svg { width: 28px; height: 28px; fill: none; stroke: currentColor; stroke-width: 1.9; stroke-linecap: round; stroke-linejoin: round; }
      .borrowing-summary-icon--active { background: rgba(55,148,95,.14); color: #188a58; }
      .borrowing-summary-icon--overdue { background: rgba(238,91,104,.16); color: #c92d3d; }
      .borrowing-summary-icon--due { background: rgba(235,177,64,.2); color: #b2760d; }
      .borrowing-summary-label { margin: 0; color: #65809a; font-size: 11px; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; }
      .borrowing-summary-value { margin: 5px 0 0; color: #1f3448; font-size: 28px; font-weight: 700; line-height: 1.05; }
      ...
      .borrowing-table { width: 100%; table-layout: fixed; border-collapse: collapse; text-align: left; }
      .borrowing-table th { padding: 12px 10px; border-top: 1px solid #edf1f5; border-bottom: 1px solid #e7edf3; color: #8495a6; font-size: 10px; font-weight: 700; letter-spacing: .6px; text-transform: uppercase; }
      .borrowing-table td { padding: 13px 10px; border-bottom: 1px solid #edf1f5; color: #40566b; font-size: 12px; vertical-align: middle; overflow: hidden; }
      .borrowing-table th:nth-child(1) { width: 9%; } .borrowing-table th:nth-child(2) { width: 17%; } .borrowing-table th:nth-child(3) { width: 19%; } .borrowing-table th:nth-child(4), .borrowing-table th:nth-child(5) { width: 11%; } .borrowing-table th:nth-child(6) { width: 12%; } .borrowing-table th:nth-child(7) { width: 11%; } .borrowing-table th:nth-child(8) { width: 10%; }
      .borrowing-cell-main strong, .borrowing-cell-main span { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .borrowing-cell-main strong { color: #294157; font-size: 12px; } .borrowing-cell-main span { margin-top: 3px; color: #8b9db1; font-size: 11px; }
      .borrowing-status { display: inline-flex; align-items: center; gap: 6px; padding: 6px 9px; border-radius: 999px; font-size: 10px; font-weight: 700; white-space: nowrap; } .borrowing-status::before { content: ""; width: 6px; height: 6px; border-radius: 50%; background: currentColor; } .borrowing-status--active { background: #eaf8ee; color: #2d8a5f; } .borrowing-status--overdue { background: #fff1f3; color: #b43348; }
      .borrowing-return-button { padding: 7px 11px; border: 0; border-radius: 7px; background: #eaf8ee; color: #2d8a5f; font: inherit; font-size: 11px; font-weight: 700; cursor: pointer; } .borrowing-return-button:hover { background: #d7f1df; }
      .borrowing-footer { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 18px 24px 24px; color: #607389; font-size: 12px; } .borrowing-pagination { display: flex; gap: 8px; } .borrowing-page-button { min-width: 34px; height: 34px; padding: 0 9px; border: 1px solid #dfeaf5; border-radius: 8px; background: #eef4fb; color: #2d536f; font: inherit; cursor: pointer; } .borrowing-page-button--active { border-color: #334e68; background: #334e68; color: #fff; } .borrowing-page-button:disabled { cursor: not-allowed; opacity: .5; }
      .borrowing-modal-backdrop { position: fixed; inset: 0; z-index: 20; display: grid; place-items: center; padding: 20px; background: rgba(15,23,42,.45); } .borrowing-modal { width: min(100%, 480px); max-height: calc(100vh - 40px); overflow: auto; border: 1px solid #dfe7ef; border-radius: 16px; background: #fff; box-shadow: 0 20px 40px rgba(15,23,42,.2); } .borrowing-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 22px 16px; border-bottom: 1px solid #edf1f5; } .borrowing-modal-header h3 { margin: 0; color: #20364a; font-size: 19px; } .borrowing-modal-header small { display: block; margin-bottom: 4px; color: #8193a6; font-size: 10px; font-weight: 700; letter-spacing: .08em; } .borrowing-close { width: 32px; height: 32px; border: 0; border-radius: 8px; background: #f3f7fb; color: #52677d; font-size: 20px; cursor: pointer; } .borrowing-steps { display: flex; gap: 8px; padding: 16px 22px 0; } .borrowing-step { flex: 1; padding: 8px; border-bottom: 2px solid #dfe7ef; color: #8193a6; font-size: 11px; font-weight: 700; text-align: center; } .borrowing-step--current { border-color: #334e68; color: #334e68; } .borrowing-modal-content { padding: 20px 22px; } .borrowing-modal-content h4 { margin: 0 0 12px; color: #20364a; font-size: 16px; } .borrowing-modal-search { width: 100%; padding: 10px 12px; border: 1px solid #dfe7ef; border-radius: 8px; background: #f8fafc; color: #102334; font: inherit; } .borrowing-modal-search:focus, .borrowing-date-grid input:focus { outline: 2px solid rgba(51,78,104,.18); border-color: #b9c8d8; background: #fff; }
      .borrowing-option { display: flex; align-items: center; gap: 10px; width: 100%; padding: 10px; border: 1px solid #e3eaf2; border-radius: 9px; background: #fff; color: #294157; text-align: left; cursor: pointer; } .borrowing-option:hover { border-color: #8bb5e2; background: #f5f9ff; } .borrowing-option-copy { min-width: 0; flex: 1; } .borrowing-option-copy strong, .borrowing-option-copy span { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .borrowing-option-copy span { margin-top: 3px; color: #8193a6; font-size: 11px; } .borrowing-summary-box { display: grid; gap: 10px; padding: 14px; border: 1px solid #e3eaf2; border-radius: 10px; background: #f8fafc; color: #40566b; font-size: 13px; } .borrowing-date-grid input { width: 100%; box-sizing: border-box; padding: 9px 10px; border: 1px solid #dfe7ef; border-radius: 8px; color: #102334; font: inherit; } .borrowing-modal-actions { display: flex; justify-content: flex-end; gap: 10px; padding: 0 22px 22px; border: 0; } .borrowing-modal-actions button { min-height: 40px; padding: 0 16px; border: 0; border-radius: 8px; font: inherit; font-weight: 700; cursor: pointer; } .borrowing-modal-secondary { background: #eef4fb; color: #2d536f; } .borrowing-modal-actions .borrowing-add-button { background: #334e68; color: #fff; }
      .borrowing-option[aria-pressed="true"] { border-color: #4a90e2; background: #f1f7ff; box-shadow: inset 3px 0 0 #4a90e2, 0 0 0 1px rgba(74,144,226,.12); }
      .borrowing-option[aria-pressed="true"] .borrowing-option-copy strong { color: #245f9e; }
      .borrowing-option[aria-pressed="true"]::after { content: "✓"; display: grid; place-items: center; flex: 0 0 20px; width: 20px; height: 20px; border-radius: 50%; background: #4a90e2; color: #fff; font-size: 12px; font-weight: 700; }
      .borrowing-option[aria-pressed="true"] .borrowing-option-avatar { background: #d8eaff; color: #245f9e; }
      .borrowing-options { display: grid; gap: 8px; max-height: 260px; margin-top: 12px; padding: 2px 4px 6px 1px; overflow-y: auto; overflow-x: hidden; scrollbar-width: thin; scrollbar-color: #c5d4e3 transparent; }
      .borrowing-options::-webkit-scrollbar { width: 6px; } .borrowing-options::-webkit-scrollbar-track { background: transparent; } .borrowing-options::-webkit-scrollbar-thumb { border-radius: 999px; background: #c5d4e3; }
      .borrowing-step--complete { border-color: #65b98a; color: #2d8a5f; }
